...
var notes = beats.selectAll('.notas')
  ...
  .append('path')
    ...
    .style('fill', 'white').attr("class","notas");


var colores = beats.selectAll('.colores')
    .data([{color:"rgb(0,0,0)"}]).enter()
  .append('path')
    .attr('d', arc2)
    .on('click', function(d){
	d.color = jQuery("input.color").css("background-color")
      d3.select(this)
          .call(colorNote)
          .style('stroke', 'black').style('fill',d.color);
	  console.log("un cli mai");
	  actualizaSecuencia();
    })
    .style('stroke-width', 1.4)
    .style('stroke', 'lightgrey')
    .style('fill',"rgb(0,0,0)").attr("class","colores");
...
function actualizaSecuencia(){
	secuencia=[]
	for (j=0;j<16;j++){
		frecuencias = []
		colorbeat=''
d3.selectAll(notes[j]).filter(function(d,i){return d.lockon==1}).each(function(d,i){frecuencias.push(d.pitch)});
colorbeat=d3.selectAll(colores[j]).style("fill");
secuencia.push({frecuencias:frecuencias,color:colorbeat})
	}
	secuenciaString = JSON.stringify(secuencia);
	jQuery.post("http://localhost:8888",secuenciaString);
}
